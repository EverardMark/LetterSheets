...
export default function Assets() {
    ...
    return (<>
        ...
        {filtered.length === 0 ? (
            <Empty icon="box" title={search || statusFilter ? "No matches" : "No assets"} desc={search || statusFilter ? "Try adjusting filters." : "Add your first fixed asset to the register."} action={search || statusFilter ? null : "Add asset"} onAction={() => setForm({ depreciation_method: "straight_line", acquisition_date: new Date().toISOString().slice(0, 10) })} />
        ) : (
...
function AssetForm({ data, categories, employees, onClose, onSave }) {
    ...
    return (
        <Modal title={isEdit ? f.name || "Edit Asset" : "Add Asset"} subtitle={isEdit ? f.asset_number : "New fixed asset"} onClose={onClose}>
            <div className="fa-modal-scroll">
                <div className="fa-f">
                    ...
                    <div className="fa-field">
                        <label className="fa-label">Depreciation Method</label>
                    ...
                    <div className="fa-field">
                        <label className="fa-label">Acquisition Cost (₱) <span className="fa-req" /></label>
                    ...
                    <div className="fa-field">
                        <label className="fa-label">Salvage Value (₱)</label>
                        ...
                        {depreciable > 0 && <span className="fa-hint">Depreciable base: {peso(depreciable)}</span>}
                    </div>
                    <div className="fa-field">
                        <label className="fa-label">Acquisition Date <span className="fa-req" /></label>
                        <input className="fa-input" type="date" value={d10(f.acquisition_date)} onChange={e => set("acquisition_date", e.target.value)} />
                    </div>
                    <div className="fa-field">
                        <label className="fa-label">In-Service Date</label>
                        <input className="fa-input" type="date" value={d10(f.in_service_date)} onChange={e => set("in_service_date", e.target.value)} />
                        <span className="fa-hint">Depreciation starts this month (defaults to acquisition)</span>
                    </div>
                    <div className="fa-field">
                        <label className="fa-label">Useful Life (months)</label>
                    ...
                    <div className="fa-field">
                        <label className="fa-label">Custodian</label>
                ...
                {isEdit && <div className="fa-hint" style={{ marginTop: 10 }}>Cost, life, salvage and method are locked once depreciation has been posted.</div>}
            </div>
        ...
        <Modal title={`${a.name}`} subtitle={`${a.asset_number} · ${a.category_name || "Uncategorised"}`} onClose={onClose}>
            <div className="fa-modal-scroll">
                <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 14, flexWrap: "wrap" }}>
                    ...
                    {a.acquisition_journal_id && <span className="fa-badge fa-b-blue">Acquisition posted</span>}
                </div>

                <div className="fa-f" style={{ marginBottom: 16 }}>
                    {[["Acquisition cost", peso(a.acquisition_cost)], ["Accumulated depreciation", peso(a.accumulated_depreciation)],
                      ["Net book value", peso(a.net_book_value)], ["Salvage value", peso(a.salvage_value)],
                      ["Acquired", d10(a.acquisition_date)], ["Periods depreciated", `${a.periods_depreciated || 0} / ${a.useful_life_months || 0}`],
                      ["Custodian", a.custodian_name || "—"], ["Location", a.location || "—"]].map(([l, v], i) => (
                ...
                {active && (
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
                        <button className="fa-btn-s" onClick={() => setAction("dispose")}><I name="minus" size={12} /> Dispose</button>
                        <button className="fa-btn-s" onClick={() => setAction("revalue")}><I name="chart" size={12} /> Revalue / Impair</button>
                ...
                <div className="fa-label" style={{ marginBottom: 6 }}>Depreciation schedule</div>
                ...
                ) : <div className="fa-hint" style={{ marginBottom: 14 }}>No depreciation posted yet.</div>}

                {/* Revaluations */}
                {revals.length > 0 && (<>
                    <div className="fa-label" style={{ marginBottom: 6 }}>Revaluations & impairment</div>
                    <div className="fa-tblwrap" style={{ marginBottom: 14 }}>
                        <table className="fa-tbl">
...
function DisposeForm({ asset, onClose, onDone, flash }) {
    ...
    return (
        <Modal title={`Dispose ${asset.asset_number}`} subtitle={`Book value ${peso(asset.net_book_value)}`} onClose={onClose}>
            <div className="fa-modal-scroll">
                <div className="fa-f">
                    ...
                    <div className="fa-field"><label className="fa-label">Proceeds (₱)</label><input className="fa-input" type="number" min="0" step="0.01" value={f.proceeds} onChange={e => set("proceeds", e.target.value)} placeholder="0.00" /></div>
                ...
                <div className="fa-hint" style={{ marginTop: 8 }}>Estimated {gain >= 0 ? "gain" : "loss"} on disposal: <b style={{ color: gain >= 0 ? "#0d9488" : "#ef4444" }}>{peso(Math.abs(gain))}</b></div>
            </div>
            <div className="fa-modal-foot"><button className="fa-btn-s" onClick={onClose}>Cancel</button><button className="fa-btn-p" onClick={submit}>Confirm Disposal</button></div>
        </Modal>
    );
}
...
function RevalueForm({ asset, onClose, onDone, flash }) {
    ...
    return (
        <Modal title={`Revalue / Impair ${asset.asset_number}`} subtitle={`Current book value ${peso(asset.net_book_value)}`} onClose={onClose}>
            <div className="fa-modal-scroll">
                <div className="fa-f">
                    ...
                    <div className="fa-field"><label className="fa-label">New carrying value (₱) <span className="fa-req" /></label><input className="fa-input" type="number" min="0" step="0.01" value={f.new_value} onChange={e => set("new_value", e.target.value)} placeholder="0.00" /></div>
                    <div className="fa-field fa-f-full"><label className="fa-label">Reason</label><input className="fa-input" value={f.reason || ""} onChange={e => set("reason", e.target.value)} placeholder="e.g. market revaluation, impairment test" /></div>
                </div>
                {f.new_value !== "" && <div className="fa-hint" style={{ marginTop: 8 }}>{delta >= 0 ? "Revaluation increase" : "Impairment"} of <b style={{ color: delta >= 0 ? "#0d9488" : "#ef4444" }}>{peso(Math.abs(delta))}</b></div>}
            </div>
        ...
        <Modal title={`Transfer ${asset.asset_number}`} subtitle={`From ${asset.location || "—"} · ${asset.custodian_name || "no custodian"}`} onClose={onClose}>
            <div className="fa-modal-scroll">
                <div className="fa-f">
                    ...
                    <div className="fa-field"><label className="fa-label">To Custodian</label>
