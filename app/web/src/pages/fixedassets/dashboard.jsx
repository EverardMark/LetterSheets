...
function FixedAssetsOverview({ nav }) {
    ...
    const cards = [
        { label: "Total Assets",     value: stats?.total_assets ?? 0,         icon: "box",    color: "#2d9e8b" },
        { label: "Net Book Value",   value: peso(stats?.net_book_value),      icon: "peso",   color: "#0ea5e9" },
        { label: "YTD Depreciation", value: peso(stats?.ytd_depreciation),    icon: "chart",  color: "#f59e0b" },
        ...
        <div className="fa-grid2">
            ...
            <div className="fa-card" style={{ marginBottom: 0 }}>
                <div className="fa-card-h">
                    <h3 className="fa-card-t">Depreciation Runs</h3>
                ...
                {runs.length > 0 ? (
                    <div className="fa-tblwrap" style={{ border: "none" }}>
                        <table className="fa-tbl">
                            <thead><tr><th>Period</th><th className="fa-tbl-r">Assets</th><th className="fa-tbl-r">Amount</th><th>GL</th></tr></thead>
                            <tbody>
                ...
                ) : <Empty icon="chart" title="No depreciation yet" desc="Run a monthly depreciation to build the schedule." />}
            </div>

            {/* Assets by status */}
            <div className="fa-card" style={{ marginBottom: 0 }}>
                <div className="fa-card-h">
                    <h3 className="fa-card-t">Register</h3>
                ...
                {assets.length > 0 ? (
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                            <span style={{ color: "#888" }}>Accumulated depreciation</span>
                            <span className="fa-tbl-mono" style={{ fontWeight: 700, color: "#d97706" }}>{peso(stats?.total_accumulated_depreciation)}</span>
                        </div>
                    </div>
                ) : <Empty icon="box" title="No assets yet" desc="Add your first asset to the register." action="Add asset" onAction={() => nav("/fixed-assets/assets")} />}
            </div>
        </div>
    </>);
}
            ...
            <div className="fa-head">
                <div className="fa-head-l">
                    ...
                    <div>
                        ...
                        <p className="fa-sub">Register, depreciation, disposal and lifecycle tracking</p>
                    </div>
                </div>
            </div>
